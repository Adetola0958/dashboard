...
let examCountDown = (text , area , hr , min ,sec) => {
    ...
    selector(area).textContent = `${hrs} : ${mins} : ${secs}` 
...
let  fetchQuestion = (elemUrl , tag) => {
    ...
    items.map((item , index) => {
        if (index == tag) {
            let questionUrl = "http://localhost:3000" + selector(elemUrl).getAttribute("data-question") + "/course/" + item._id + "/question" 
...
*/ 
let testPath = "http://localhost:3000" + selector("#testRun").getAttribute("data-href") 
...
startButton.addEventListener("click" , event => { 
    ...
    examCountDown("#timeText" , "#time" , 0 , timer , 59) 
    fetchQuestion("#testRun" , 0)
    displayQuestion(0)
    ...
    if (event.target.id == "FINISHED") {
        ...
        try {
            if (localStorage.response && localStorage.question) {
                ...
                examCountDown("#timeText" , "#time" , 0 , 1 , 0) 
            }else {
                throw {
